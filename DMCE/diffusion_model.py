...
# Optional import for FID score calculation
try:
    from pytorch_fid.inception import InceptionV3
    _HAS_PYTORCH_FID = True
except ImportError:
    InceptionV3 = None
    _HAS_PYTORCH_FID = False
    ...
    def __init__(self,
        ...
        if track_fid_score:
            if mode == '1D' and self.complex_data:
                ...
                if not _HAS_PYTORCH_FID:
                    raise ImportError(
                        "pytorch_fid is required for FID score calculation in 2D mode. "
...
class Tester(object):
    ...
    def _test_nmse(self) -> dict:
        """
        Test function for the NMSE criterion. For different SNR values between -10 and 40 dB, the test data is corrupted
        ...
        # specify which SNRs should be evaluated (in dB)
        ...
        timings_sec = []          # TIME FOR EACH SNR
        steps_list = []           # START STEP FOR EACH SNR
        tps_ms_list = []          # TIME PER SAMPLE FOR EACH SNR
        ...
        with torch.no_grad():
            # iterate over linear SNR values (ρ)
            ...
            for snr_idx, snr in enumerate(tqdm(iterable=snr_range, desc="SNR sweep")):
                ...
                # choose closest diffusion time index t_hat for logging only
                t_hat = int(torch.abs(self.model.snrs - snr).argmin())
                steps_list.append(int(t_hat))
                
                # Print current SNR for better visibility (especially if tqdm stalls)
                if self.use_dps:
                    print(f"\n[SNR {snr_db:.1f} dB] Processing with DPS (lambda={self.dps_lambda:.3f})...")
                else:
                    print(f"\n[SNR {snr_db:.1f} dB] Processing baseline...")

                # ----- build DPS sampler for this SNR (if enabled) -----
                if self.use_dps:
                    rho = float(snr)  # snr_range is already ρ (linear SNR)

                    # CRITICAL: Compute sigma_y^2 to match the AWGN noise variance
                    # DMCE's AWGN function: y = x + (1/sqrt(ρ)) * multiplier * n
                    # where n ~ N(0, I), so Var[noise] = (multiplier^2) / ρ
                    # This sigma_y2_snr must match the actual noise variance used to corrupt y
                    noise_mult = float(self.model.noise_multiplier)
                    sigma_y2_snr = (noise_mult ** 2) / rho

                    # Build likelihood gradient function for this SNR
                    # For AWGN: ∇_x log p(y|x) = (y - x) / sigma_y^2
                    likelihood_grad_fn = make_awgn_likelihood_grad(sigma_y2_snr)

                    # DPS should use deterministic reverse steps (DDIM-style) for better performance
                    # Setting add_random=False makes the sampling deterministic, which is crucial for DPS
                    dps_sampler = DpsSampler(
                        dm=self.model,
                        likelihood_grad_fn=likelihood_grad_fn,
                        lambda_dps=self.dps_lambda,   # Tester passes dps_lambda here
                        sigma_y2=sigma_y2_snr,
                        add_random=False,  # CRITICAL: DPS works better with deterministic steps
                    )
                else:
                    dps_sampler = None
                ...
                for batch_idx, data_batch in enumerate(self.dataloader):
                    try:
                        ...
                        # functional.awgn expects linear SNR (ρ), which snr already is
                        y = functional.awgn(
                            data_batch,
                            snr,
                            multiplier=self.model.noise_multiplier
                        )
                        ...
                        print(f"\nERROR at SNR {snr_db:.1f} dB, batch {batch_idx}: {e}")
                        import traceback
                        traceback.print_exc()
                        # Clean up and re-raise to stop the experiment
                        if self.device.type == 'cuda':
                            torch.cuda.empty_cache()
                        raise

                    # optional FFT post-processing
                    if self.fft_pre:
                        if self.return_all_timesteps:
                            x_est = ut.complex_1d_fft(
                                x_est,
                                ifft=True,
                                mode=self.mode,
                                _4d_array=True
                    ...
                    # Clean up GPU memory after each batch (especially important for DPS)
                ...
                # Clean up DPS sampler and GPU memory after each SNR
                if self.use_dps:
                    del dps_sampler
                    if self.device.type == 'cuda':
                        torch.cuda.empty_cache()

                # ----- NMSE computation (same as original code) -----
                if self.return_all_timesteps:
        ...
        return {
            ...
            'Timings_sec': timings_sec,          # TIME FOR EACH SNR
            'Time_per_sample_ms': tps_ms_list,   # TIME PER SAMPLE FOR EACH SNR
            'Steps': steps_list,                 # START STEP FOR EACH SNR
        }
