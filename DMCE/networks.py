...
def get_positional_embedding(t: torch.Tensor, dim: int) -> torch.Tensor:
    """
    Creates the time embedding from a continuous time value
    Supports both discrete (integer) and continuous (float) time steps

    Parameters
    ----------
    t : Tensor of shape [batch_size]
        timesteps of the corresponding data samples (can be continuous)
    dim : int
    ...
    if half_dim <= 1:
        ...
        if dim == 1:
            ...
            emb = torch.zeros(t.shape[0], dim, device=t.device)
            emb[:, 0] = t
            if dim > 1:
                emb[:, 1] = t * 0.1
            return emb
...
class CNN(nn.Module):
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
        ...
        # Reshape scale and shift to match x dimensions
        if self.mode == '1D':
            # For 1D: x shape is [batch, channels, length]
            scale = scale.unsqueeze(-1)  # [batch, channels, 1]
            shift = shift.unsqueeze(-1)  # [batch, channels, 1]
        else:
            # For 2D: x shape is [batch, channels, height, width]
